...
class GRU_VAE(nn.Module):
    def __init__(self, zp_dims, zr_dims, pf_num, inpaint_len, gen_dims, vae_model):
        ...
        # Capa LSTM
        self.p_gru = nn.GRU(zp_dims * 2, gen_dims, pf_num, batch_first = True, bidirectional = True, dropout = 0.5)
        self.r_gru = nn.GRU(zr_dims * 2, gen_dims, pf_num, batch_first = True, bidirectional = True, dropout = 0.5)

        self.gen_p_gru = nn.GRU(zp_dims, gen_dims, pf_num, batch_first = True, bidirectional = True, dropout = 0.5)
        self.gen_r_gru = nn.GRU(zr_dims, gen_dims, pf_num, batch_first = True, bidirectional = True, dropout = 0.5)
    ...
    def get_pf_pz(self, past_pz, future_pz):
        ...
        # Se podria usar esta sobre la concatenacion de past y futuro en vez de dos y luego concatenar
        pz = torch.cat((past_pz, future_pz), 2)
        _, h = self.p_gru(pz)
        pf_pz = h

        #_, h_past_pz = self.past_p_gru(past_rz)
        #_, h_future_pz = self.future_p_gru(future_rz)
        #pf_pz = torch.cat((h_past_pz, h_future_pz), 2)
        return pf_pz
    ...
    def get_pf_rz(self, past_rz, future_rz):
        ...
        #_, h_past_rz = self.past_r_gru(past_rz)
        #_, h_future_rz = self.future_r_gru(future_rz)
        #pf_rz = torch.cat((h_past_rz, h_future_rz), 2)
