...
class Note():
    ...
    def to_remi(self):
        return np.array([self.measure, self.rel_pos, self.duration, self.pitch, self.velocity, self.tempo])
...
def add_train_val_test(df):
    ...
    val = [{"filename": f, "set": "val"} for f in files[len(train): len(train)+int(n*0.1)]]
